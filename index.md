...
## Summary
For this milestone, I implemented multiple different functions of the robot. The robot it now able to follow a black line on the ground using the line track module. It detects the color of the floor underneath the robot, and returns an integer based on the color (0 for white, 1 for black). If 1 is returned, the robot turns forward right, and if 2 is returned the robot turns forward left, causing it to zigzag along and follow the line. The car can also detect distance and avoid obstacles using the obstacle avoidance module and ultrasonic sensors. The obstacle avoidance modules send a signal if they detect objects ahead, and the detection distance is changed by rotating a potentiometer on the module. Another function that utilized the obstacle avoidance module was hand-following. The modules, upon detection of an object, turned and went towards it instead of away, allowing the car to follow the movements of my hand. The ultrasonic sensor sends ultrasonic waves ahead, and records how long it takes for the waves to rebound off an object in order to calculate the distance between the sensor and that object. It has longer range than the obstacle avoidance modules, which are only effective in close quarters. Using these tools, the robot was able to go fully autonomous; when it deteced an object, it turned and went in a different direction. The most important part of this milestone was the IR remote controller. It sends a signal to an IR reciever on the bread board of the car, which then transmits a signal to the Arduino allowing it to perform a task based on the button pressed. The remote is encoded with basic movements (move forwards, move backwards, stop, turn left, turn right, etc) along with the previously mentioned functions (autonomous, line and hand following, etc). 

![Headstone Image](FinalRobotWiring.png)
